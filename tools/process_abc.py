...
def process_step_folder(data_root, output_root, step_ids, brep_sample_resolution, point_cloud_sample_num, scanned_pc_n_points, base_seed=None):
    ...
    for i, step_id in enumerate(step_ids):
        ...
        try:
            ...
            solid_processor.export_solid(output_root / step_id / "normalized_shape.step")
            solid_processor.export_mesh(output_root / step_id / "mesh.stl")
            solid_processor.export_point_cloud(output_root / step_id / "pc.ply", sample_num=point_cloud_sample_num)
            solid_processor.export_uv_grids(output_root / step_id / "uv_grids.npz", sample_resolution=brep_sample_resolution)
            solid_processor.export_point_cloud_numpy(output_root / step_id / "pc.npz")
            solid_processor.export_random_cropped_pc(output_root / step_id / "cropped_pc.h5")
            solid_processor.export_scanned_point_cloud(output_root / step_id / "back_projection_pc_cube.npz", strategy='cube', n_points=scanned_pc_n_points)
            solid_processor.export_scanned_point_cloud(output_root / step_id / "back_projection_pc_sphere.npz", strategy='sphere', n_points=scanned_pc_n_points)
            solid_processor.export_photos(output_root / step_id / "photos.npz")
